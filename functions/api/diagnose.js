...
2. recommended_plan と recommended_service は必ず整合させること。以下の対応ルールを厳守：
   ...
   【梅プラン】真心お墓参り代行 = 合掌・簡易清掃・献花・写真報告のみ
   - 選ぶ条件: 劣化がほぼなく、定期訪問・お参り代行で十分な状態（主に評価A）
   - recommended_service には「合掌」「簡易清掃」「献花」「写真報告」以外の施工内容を書かないこと
   - 「洗剤洗浄」「コケ除去」「水垢除去」「撥水コーティング」「研磨」は絶対に含めない
   ...
   - 「撥水コーティング」「特殊研磨」「再生研磨」は絶対に含めない（これらが必要なら松プランにすること）

   【松プラン】プレミアム再生研磨 = 竹の内容 + 特殊研磨・撥水コーティング
   - 選ぶ条件: 重度の劣化、深いシミ、風化、色ムラが目立つ、または中度でも長期美観維持のため撥水コーティングが必要な状態（主に評価D、または評価Cで撥水コーティング推奨の場合）
   - recommended_service に「撥水コーティング」「特殊研磨」「再生研磨」を含める場合は必ず松プランを選ぶこと
...
function reconcilePlanWithService(r) {
  ...
  // 整合性チェック：施工内容が松相当なら松に格上げ
  if (hasMatsuWork) return "松";

  // 施工内容が竹相当以上 → 竹以上に確定
  if (hasTakeWork) {
    // AIが松と言っていたら松を尊重（ただし松ワードがなければ竹に下げる可能性もあるが、基本尊重）
